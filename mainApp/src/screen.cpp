...
void screen::render(scenes::Scene pseudo, bool bUseFbo){
...
//                int imageID = busses[0];
                        ...
                        ofPushMatrix();
                            ofTranslate(width * 0.5, height * 0.5);
                                if(i <= (NUM_INSTRUCTIONS-1)){
                                    if(i > 0){
                                        ofTranslate((width - instructions[i].getWidth()) * 0.5, (height - instructions[i].getHeight()) * 0.5);
                                        ofScale(0.8);
                                        ofTranslate((width - instructions[i].getWidth()) * -0.5, (height - instructions[i].getHeight()) * -0.5);
                                    }
            ...
            case scenes::Countdown:{
                ...
                if(count > 0){
                    ...
                    countFont.drawString(ofToString((int)floor(count)), width*0.5 - r.getWidth() * 0.5, height*0.5 + r.getHeight() * 0.5);
                }
            }
                break;
            case scenes::Stars:{
...
//                        ofDrawLine(0, 0, width * 0.25, 0);
            ...
            case scenes::ReturnToShip:{
                ...
                if(imgIndex < NUM_RETURNIMAGES && imgIndex >= 0){
                    returnImages[imgIndex].draw(0,0, width, height);
                }
            }
                break;
            ...
            case scenes::Waveform:{
                if(busses[0]){
                    ...
                    ofNoFill();
                    ofPushMatrix();
                    ofTranslate(0, -height*0.2); // Waveform @ 25% of screen H
                    ofBeginShape();
                    for (unsigned int i = 0; i < waveForm.size(); i++){
                        ofVertex(
                                 (i*((float)width/waveForm.size())),
                                 waveForm[i]
                                 );
                    }
                    ofEndShape(false);
            ...
            case scenes::Code:{
                ofPushMatrix();
                ofTranslate(glm::vec2(width, height)*0.5);
                ofScale(1.4);
                ofTranslate(glm::vec2(width, height)*-0.5);
    ...
    //                    ofScale(1/1.1);
            ...
            case scenes::Autopilot:{
                ...
                if(prefix == "1280/"){
                    ofDrawRectangle(389, 536 * 0.666, (busses[1]/100.)*(889-389), height); // 66%
                } else{
                    ofDrawRectangle(416, 536, (busses[1]/100.)*(866-416), height);
                }
                images["Autopilot"].draw(0,0, width, height);
                joniskHover(false);
    ...
    //                ofRectangle box = autoPilotFont.getStringBoundingBox(str, 0, 0);
            ...
            case scenes::Test:{
                ...
                float lineW = 20;
                ofSetColor(0);
                ofDrawRectangle(lineW, lineW, width - lineW*2, height - lineW*2);
                ofSetColor(255);
//                ofDrawBitmapString(ofToString(portNumAdd), lineW*2, lineW*2);
                string idTxt = ofToString(portNumAdd);
                countFont.drawString(idTxt, width*0.5 - countFont.getStringBoundingBox(idTxt, 0, 0).width*0.5,
                                     height*0.5 + countFont.getStringBoundingBox(idTxt, 0, 0).height*0.5);
            }
                break;
            case scenes::Einde:
...
void screen::processMsg(ofxOscMessage &m){
    ...
    ofPushMatrix();
        ...
        ofSetColor(255);
            ofPushMatrix(); // Note: the zoom is not fixed to jonisk center
                ...
                if(bGradient)
                    images["Intro gradient"].draw(-width*0.5, -height*0.25, width, height);
            ofPopMatrix();
        ofSetColor(255);
